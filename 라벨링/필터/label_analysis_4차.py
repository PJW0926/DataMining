...
# 목적
# =========================================================
# - 수동 라벨링 데이터(all_라벨링_4차.csv)에 규칙 기반 진정성 필터 적용
...
# - ALL 전체 데이터 평가를 추가하여 3차/4차 라벨 수정 효과를 같은 기준으로 비교
#
# 라벨 기준:
...
OUTPUT_DIR = BASE_DIR / "filter_result_trust_score_event_pattern_all_eval"
...
# 전체 데이터 기준 결과도 함께 생성
# train/test는 코드 실행 중 임시로 나뉘는 검증용 분할이므로,
# 3차와 4차 라벨 수정 효과를 직접 비교하려면 ALL 기준을 함께 보는 것이 좋음
...
print("\n===== ALL trust_score 분포 =====")
print(all_result_df["trust_score"].value_counts().sort_index().to_string())
...
existing_all_wrong_cols = [col for col in (["split"] + wrong_cols) if col in all_wrong.columns]

# 전체 데이터 기준 오분류: 3차/4차 라벨 수정 효과 비교용
all_wrong[existing_all_wrong_cols].to_csv(
    OUTPUT_DIR / "all_wrong_cases.csv",
    index=False,
    encoding="utf-8-sig"
)

# test 기준 오분류: 일반화 성능 확인용
...
save_confusion_matrix_png(all_cm, "ALL Confusion Matrix", "confusion_matrix_all.png")
...
with open(summary_path, "w", encoding="utf-8") as f:
    ...
    f.write("[성능]\n")
    f.write("ALL은 전체 수동 라벨링 데이터 기준 성능이며, 3차/4차 라벨 수정 효과 비교에 가장 적합합니다.\n")
    f.write("TRAIN/TEST는 검증용 분할 기준 성능입니다.\n\n")
