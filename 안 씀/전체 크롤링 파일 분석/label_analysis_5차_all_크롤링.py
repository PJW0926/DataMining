...
# =========================================================
# Kakao review trust-level analysis
# - kakao_raw 폴더 안의 카카오 크롤링 파일 전체 병합
# - manual_label이 있으면 성능평가까지 수행
# - manual_label이 없으면 trust_score / trust_level / trust_weight만 산출
# =========================================================
...
# 카카오 크롤링 파일들을 넣을 폴더
DATA_DIR = BASE_DIR / "kakao_all"

# 결과 저장 폴더
OUTPUT_DIR = BASE_DIR / "kakao_filter_result"
...
if not csv_files and not xlsx_files:
    raise FileNotFoundError(
        f"{DATA_DIR} 폴더에 카카오 크롤링 csv/xlsx 파일이 없습니다.\n"
        f"카카오 크롤링 파일들을 kakao_raw 폴더에 넣어 주세요."
    )

df_list = []

for file in csv_files:
    temp = read_csv_safely(file)
...
print("\n===== 카카오 파일 병합 완료 =====")
print(f"불러온 파일 수: {len(df_list)}개")
...
HAS_LABEL = "manual_label" in df.columns

if not HAS_LABEL:
    print("\n[주의] manual_label 컬럼이 없습니다.")
    print("성능평가, threshold tuning, confusion matrix, Random Forest 검증은 건너뜁니다.")
    print("대신 리뷰별 trust_score, trust_level, trust_weight만 산출합니다.")
...
trust_level_summary.to_csv(
    OUTPUT_DIR / "kakao_trust_level_summary.csv",
    index=False,
    encoding="utf-8-sig"
)
...
all_result_df.to_csv(
    OUTPUT_DIR / "kakao_all_filter_result.csv",
    index=False,
    encoding="utf-8-sig"
)
...
with open(summary_path, "w", encoding="utf-8") as f:
    f.write("카카오 크롤링 리뷰 trust-level 기반 진정성 필터 결과\n")
    f.write("=" * 70 + "\n\n")

    f.write("[입력 데이터]\n")
    f.write(f"- 입력 폴더: {DATA_DIR}\n")
    f.write(f"- 불러온 파일 수: {len(df_list)}개\n")
    f.write(f"- 빈 리뷰 제거 후 전체 리뷰 수: {len(all_result_df)}개\n")
    f.write("- platform은 kakao로 통일함\n\n")
    ...
    if HAS_LABEL:
        ...
        f.write("[주의]\n")
        f.write("- manual_label 컬럼이 없어 성능평가를 수행하지 않음\n")
        f.write("- Accuracy, F1-score, Confusion Matrix, Threshold Tuning, Random Forest 검증은 생략함\n\n")
...
print("- kakao_all_filter_result.csv")
print("- kakao_trust_level_summary.csv")
