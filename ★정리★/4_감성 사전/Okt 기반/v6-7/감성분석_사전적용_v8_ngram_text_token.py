...
platform_output = BASE_DIR / "platform_category_sentiment_summary_v8.csv"
store_output = BASE_DIR / "store_category_sentiment_summary_v8.csv"
review_score_output = BASE_DIR / "review_sentiment_scores_v8.csv"
...
def read_csv_safely(path):
    """utf-8-sig, cp949, euc-kr 순서로 CSV를 읽습니다."""
...
# 원문 매칭용: 긴 표현부터 먼저 확인
# 단어 길이가 긴 것 우선. 띄어쓰기는 제거하고 길이 계산.
dict_terms_sorted = sorted(
    sentiment_map.keys(),
    key=lambda x: len(str(x).replace(" ", "")),
    reverse=True,
)
...
# =========================================================
# 방식:
# 1) 원문 review_text에서 사전 표현을 먼저 찾음
#    - 고성방가, 비싸도 맛있, 맛을 모르 같은 원문 표현 보존용
# 2) tokens에서 n-gram 표현을 찾음
#    - 맛있다, 비싸다, 고성 방가, 별로 사람 모르다 같은 토큰 표현 탐지용
# 3) 긴 표현을 먼저 매칭함
#    - 3어절 표현이 있으면 내부 1어절/2어절은 중복 계산하지 않음


def analyze_review(review_text, tokens_text):
    text = normalize_text_for_match(review_text)
    ...
    # 원문에서 긴 표현과 짧은 표현이 중복 매칭되는 것 방지
    used_char_idx = set()
    ...
    # =====================================================
    # 1단계: 원문 review_text 기반 매칭
    # =====================================================
    for term in dict_terms_sorted:
        term = str(term).strip()
        if not term:
            continue
        ...
        # 1글자는 원문 substring 매칭 시 오탐이 너무 많아서 제외
        # 예: 맛, 양, 값 등은 tokens 단계에서 잡음
        if len(term_no_space) < 2:
            continue

        start = text_no_space.find(term_no_space)

        if start == -1:
            continue

        end = start + len(term_no_space)
        char_range = set(range(start, end))

        # 이미 더 긴 표현에 포함된 경우 스킵
        if used_char_idx & char_range:
            continue

        for category, score in sentiment_map[term]:
            if category in scores:
                scores[category] += score
                matched[category].append(f"{term}:{score}[text]")

        matched_terms.add(term)
        used_char_idx.update(char_range)

    # =====================================================
    # 2단계: tokens 기반 n-gram 매칭
    # =====================================================
    # 긴 표현부터 먼저 매칭
    for n in range(max_ngram, 0, -1):
        for i in range(len(tokens) - n + 1):
            ...
            # 원문에서 이미 같은 표현을 잡았다면 중복 계산하지 않음
...
result_df = df.apply(
    lambda row: analyze_review(row["review_text"], row["tokens"]),
    axis=1,
)
