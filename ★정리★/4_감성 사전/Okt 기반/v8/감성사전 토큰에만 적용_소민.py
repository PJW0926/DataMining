...
output_file = BASE_DIR / "final_high_trust_reviews_category_sentiment_v8_소민_3.csv"
...
# =========================================================
# 방식:
# 1) review_text 원문은 매칭에 사용하지 않음
# 2) tokens 열에서만 n-gram 표현을 찾음
#    - 맛있다, 비싸다, 고성 방가, 별로 사람 모르다 같은 토큰 표현 탐지용
# 3) 긴 표현을 먼저 매칭함
#    - 3어절 표현이 있으면 내부 1어절/2어절은 중복 계산하지 않음
# 4) 출력 라벨은 [token]만 사용함


def analyze_review(review_text, tokens_text):
    # review_text는 결과 파일에 원문을 보존하기 위해 남겨두지만,
    # 감성사전 매칭에는 사용하지 않습니다.
    ...
    # 이미 점수화한 표현 기록
    # 같은 리뷰 안에서 같은 사전 표현이 여러 번 잡혀도 한 번만 계산합니다.
    matched_terms = set()
