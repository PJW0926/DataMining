...
INPUT_FILE = BASE_DIR / "kakao_크롤링.csv"

# 결과 저장 폴더
OUTPUT_DIR = BASE_DIR / "result_kakao_crawling"
...
if not INPUT_FILE.exists():
    raise FileNotFoundError(
        ...
        f"이 파이썬 파일과 같은 폴더에 all_크롤링.xlsx를 넣어 주세요."
    )
...
# 전체 크롤링 데이터가 네이버/카카오 혼합일 수 있으므로 특정 플랫폼으로 덮어쓰지 않는다.
...
# 전체 크롤링 적용용 코드이므로 manual_label은 사용하지 않는다.
# 실수로 manual_label 컬럼이 남아 있어도 성능평가에 쓰지 않도록 제거한다.
if "manual_label" in df.columns:
    df = df.drop(columns=["manual_label"])

HAS_LABEL = False

print("\n[안내] 전체 크롤링 적용 모드입니다.")
print("manual_label을 사용하지 않으므로 Accuracy/F1/오분류 평가는 수행하지 않습니다.")
print("대신 리뷰별 trust_score, trust_level, pred_label, trust_reasons를 산출합니다.")
...
trust_level_summary.to_csv(
    OUTPUT_DIR / "crawling_trust_level_summary.csv",
    index=False,
    encoding="utf-8-sig"
)
...
all_result_df.to_csv(
    OUTPUT_DIR / "crawling_all_filter_result.csv",
    index=False,
    encoding="utf-8-sig"
)

# 최종 진짜 리뷰 후보 저장
pred_real_reviews = all_result_df[all_result_df["pred_label"] == 1].copy()
pred_real_reviews.to_csv(
    OUTPUT_DIR / "final_pred_real_reviews.csv",
    index=False,
    encoding="utf-8-sig"
)

high_trust_reviews = all_result_df[all_result_df["trust_level"] == "high"].copy()
high_trust_reviews.to_csv(
    OUTPUT_DIR / "final_high_trust_reviews.csv",
    index=False,
    encoding="utf-8-sig"
)
...
platform_summary = (
    ...
    .agg(
        ...
        median_trust_score=("trust_score", "median"),
        low_ratio=("trust_level", lambda x: (x == "low").mean()),
        ambiguous_ratio=("trust_level", lambda x: (x == "ambiguous").mean()),
        high_ratio=("trust_level", lambda x: (x == "high").mean()),
        pred_real_ratio=("pred_label", lambda x: (x == 1).mean()),
    )
    .round(4)
    .reset_index()
)
platform_summary.to_csv(
    OUTPUT_DIR / "platform_trust_summary.csv",
    index=False,
    encoding="utf-8-sig"
)
...
store_summary = (
    ...
    .round(4)
    .reset_index()
    .sort_values(["platform", "mean_trust_score"], ascending=[True, False])
)
store_summary.to_csv(
    OUTPUT_DIR / "store_trust_summary.csv",
    index=False,
    encoding="utf-8-sig"
)
...
with open(summary_path, "w", encoding="utf-8") as f:
    f.write("전체 크롤링 리뷰 trust-level 기반 진정성 필터 적용 결과\n")
    f.write("=" * 70 + "\n\n")
    ...
    f.write("- 전체 크롤링 데이터에는 manual_label을 사용하지 않으므로 Accuracy/F1/오분류 평가는 수행하지 않음\n")
    ...
    f.write("[플랫폼별 trust summary]\n")
    f.write(platform_summary.to_string(index=False))
    f.write("\n\n")

    f.write("[최종 리뷰 후보 수]\n")
    f.write(f"- pred_label=1 리뷰 수: {len(pred_real_reviews)}\n")
    f.write(f"- trust_level=high 리뷰 수: {len(high_trust_reviews)}\n")
...
print("- crawling_all_filter_result.csv")
print("- crawling_trust_level_summary.csv")
print("- pred_label_feature_summary.csv")
print("- trust_level_feature_summary.csv")
print("- final_pred_real_reviews.csv")
print("- final_high_trust_reviews.csv")
print("- platform_trust_summary.csv")
print("- store_trust_summary.csv")
print("- 분석_요약.txt")
